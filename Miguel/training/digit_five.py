...
import torch
from Miguel.training.methods import supervised_learning
import scipy.io


class Digit_Five_Datasheet():
    def __init__(self, num, c, transform=None) -> None:
        ...
        self.labels_root = []
        for i in num:
            dict_mat = scipy.io.loadmat(c.DATA_UBI + 'data_' + str(i) + '.mat')
...
class Digit_Five_training():
    def __init__(self, c, p=1) -> None:
        ...
        self.LR = c.LR
        self.BACH_SIZE = c.BACH_SIZE

        self.device = c.DEVICE
        self.epochs = c.EPOCH

        self.sl = supervised_learning(c, self)
        self.net = None
        self.dba_index = None
        self.byz = False
    ...
    def create_train(self):
        ...
        self.df_train = Digit_Five_Datasheet(self.train_indices, self.c, self.transform)
        self.trainloader = torch.utils.data.DataLoader(self.df_train, batch_size=self.BACH_SIZE,
                                                       shuffle=True, num_workers=2,
                                                       generator=torch.Generator(device='cuda'))

    def create_test(self):
        print(f'[INFO USER {self.c.RANK}] Creando testloader')
        self.df_test = Digit_Five_Datasheet(self.c.TEST_INDEX_LOCAL, self.c, self.transform)
        self.testloader = torch.utils.data.DataLoader(self.df_test, batch_size=self.BACH_SIZE,
                                                      shuffle=False, num_workers=2,
                                                      generator=torch.Generator(device='cuda'))

        self.df_test = Digit_Five_Datasheet(self.c.TEST_INDEX_GLOBAL, self.c, self.transform)
        self.testloader_global = torch.utils.data.DataLoader(self.df_test, batch_size=self.BACH_SIZE,
                                                             shuffle=False, num_workers=2,
                                                             generator=torch.Generator(device='cuda'))


if __name__ == "__main__":
    from Miguel.config import config

    c = config()
    df = Digit_Five_training(c)
    # df.train()
