...
def show_batch(data, show_function, num_images=3):
    images, labels = data
...
def mostrar_imaxes_rand(aprendedores, undetected_byz_index, ben_workers):
    byz_rand = np.random.choice(undetected_byz_index, 1)[0]
    ben_rand = np.random.choice(ben_workers, 1)[0]
    show_batch(aprendedores[byz_rand].pre_data, show, num_images=3)
    show_batch(aprendedores[ben_rand].pre_data, show, num_images=3)


def mostrar_imaxes(aprendedores):
    for ap in aprendedores:
        show_batch(ap.pre_data, show, num_images=3)
...
def fl_detector(c, args, total_clients, entrenamento, original_clients, byz_workers):
    ...
    para_string = ("nepochs: " + str(c.EPOCH) + ", lr: " + str(c.LR) + ", batch_size: " + str(
        c.BATCH_SIZE) + ", batch_test_size: " + str(c.BATCH_TEST_SIZE) + ", nworkers: " + str(
        c.SIZE) + ", nbyz: " + str(len(byz_workers)))

    # timestamp = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
    path = os.path.join('PROBAS/D5/4b', args.home_path, f"{args.data_type}_{args.extra_data_type}",
                        str(entrenamento))
    if not os.path.exists(path):
        os.makedirs(path)
    ataques_path = path + '/at_det.txt'
    ...
    with device:
        ...
        for i in range(len(total_clients)):
            c_ = copy.deepcopy(c)
            c_.RANK = i
            c_.recalculate_db_2bases(args.data_type, args.extra_data_type)
        ...
        # MOSTRAR PORCENTAXES E IMAXES
        # porcentaxes([ap.pre_data[1] for ap in aprendedores])
        # mostrar_imaxes(aprendedores)
        ...
        for e in range(epochs):
            ...
            if (e + 1) % 50 == 0 and e > c.FLDET_START:
