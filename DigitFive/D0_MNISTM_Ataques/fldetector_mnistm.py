...
from DigitFive.D0_MNISTM_Ataques.arquivos import *
from DigitFive.byzantine import select_byzantine_range
...
def fl_detector(c, args, total_clients, entrenamento, original_clients, byz_workers):
    ...
    para_string = "nepochs: " + str(c.EPOCH) + ", lr: " + str(
        c.LR) + ", batch_size: " + str(c.BATCH_SIZE) + ", nworkers: " + str(
        c.SIZE) + ", nbyz: " + str(len(byz_workers))

    timestamp = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
    path = os.path.join('PROBAS/D5/Ataques', args.home_path, timestamp, args.byz_type, str(entrenamento))
    ...
    with device:
        ...
        for e in range(epochs):
            ...
            if (e + 1) % 25 == 0:
                testear_precisions(testloader_global, global_net, device, e, precision_array, path, target_backdoor_dba,
                                   args.byz_type)
