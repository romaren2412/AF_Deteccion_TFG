...
class DigitFiveTraining:
    ...
    def create_global_test(self, global_test_indices):
        print(f'[INFO: Creando testloader global')
        self.df_test = DigitFiveDatasheet(global_test_indices, self.c, self.transform)
        a = torch.utils.data.DataLoader(self.df_test, batch_size=self.BATCH_TEST_SIZE,
                                           shuffle=True, num_workers=2,
                                           generator=torch.Generator(device='cuda'))
        return next(iter(a))
...
class SupervisedLearning:
    ...
    def adestrar(self, c, criterion, global_net):
