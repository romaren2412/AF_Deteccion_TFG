...
def fltrust_mnist(c, total_clients, byz_workers):
    ...
    path = os.path.join('PROBAS/', c.home_path, f"{c.data_type}_{c.extra_data_type}")
    ...
    with device:
        ...
        for e in range(c.EPOCH):
            ...
            if (e + 1) % 100 == 0:
                testear_precisions_un_batch(aprendedores, testloader_global, global_net, device, e, precision_array,
                                            path, c.data_type, c.extra_data_type)
            """
            if (e + 1) % 50 == 0:
                local_precisions.append(local_precisions_ep)
            """

    resumo_final_un_batch(testloader_global, global_net, device)
