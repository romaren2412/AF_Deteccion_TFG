...
def fld_mnist_usps(args, total_clients, entrenamento, original_clients):
    ...
    path = os.path.join('MNIST_USPS/PROBAS', args.home_path, timestamp)
    ...
    with device:
        ...
        ########################################################################################################
        # CARGA DO DATASET
        train_data_loader_mnist, train_data_loader_usps = preparar_datos_mnist_usps(len(total_clients), args)
        combined_test_loader = preparar_test()
        ...
        each_worker_data, each_worker_label = repartir_datos(args, train_data_loader_mnist, train_data_loader_usps,
                                                             num_workers, device, undetected_byz_index)
        ...
        for e in range(epochs):
            ...
            if len(malicious_score) > 10 and args.tipo_exec != 'no_detect':
                ...
                if det is not None:
                    datos_finais(path, train_acc_list, combined_test_loader, net, device, e, malicious_score,
                                 undetected_byz_index)
                    return det
            ...
            if (e + 1) % 10 == 0:
                testear_precisions(combined_test_loader, net, device, e, train_acc_list, path)
    ...
    resumo_final(combined_test_loader, net, device, e, malicious_score, path, undetected_byz_index)
